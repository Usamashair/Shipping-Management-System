...
/**
 * Proxies FedEx Track Document API: POST /track/v1/trackingdocuments.
 *
 * @see https://developer.fedex.com/api/en-us/catalog/track/v1/docs.html
 */
class FedExTrackDocumentService
{
    ...
    /**
     * @param  array<string, mixed>  $payload  FedEx request body (trackDocumentDetail, trackDocumentSpecification, etc.)
     * @return array<string, mixed>
     */
    public function request(array $payload, ?string $customerTransactionId = null, ?string $locale = null): array
    {
        ...
        try {
            ...
            Log::error('FedEx OAuth failed before track document.', [
        ...
        $response = FedExHttp::pending($timeout)
            ...
            ->withToken($token)
            ->post($base.'/track/v1/trackingdocuments', $payload);
        ...
        if (! $response->successful()) {
            ...
            Log::error('FedEx track document HTTP error.', [
                'x-customer-transaction-id' => $txnHeader,
    ...
    private function firstFedExErrorMessage(array $normalizedErrors, array $body, int $httpStatus): string
    {
        ...
        return 'FedEx track document request failed (HTTP '.$httpStatus.').';
    }
}
