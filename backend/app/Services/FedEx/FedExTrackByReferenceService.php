...
/**
 * Proxies FedEx Track by References API: POST /track/v1/referencenumbers.
 *
 * @see https://developer.fedex.com/api/en-us/catalog/track/v1/docs.html
 */
class FedExTrackByReferenceService
{
    ...
    /**
     * @param  array<string, mixed>  $payload  FedEx request body (referencesInformation, includeDetailedScans, etc.)
    ...
    public function track(array $payload, ?string $customerTransactionId = null, ?string $locale = null): array
    {
        ...
        try {
            ...
            Log::error('FedEx OAuth failed before track by reference.', [
        ...
        $response = FedExHttp::pending($timeout)
            ...
            ->withToken($token)
            ->post($base.'/track/v1/referencenumbers', $payload);
        ...
        if (! $response->successful()) {
            ...
            Log::error('FedEx track by reference HTTP error.', [
                'x-customer-transaction-id' => $txnHeader,
    ...
    private function firstFedExErrorMessage(array $normalizedErrors, array $body, int $httpStatus): string
    {
        ...
        return 'FedEx track by reference failed (HTTP '.$httpStatus.').';
    }
}
