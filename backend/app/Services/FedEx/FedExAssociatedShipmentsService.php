...
/**
 * Proxies FedEx Track Multiple Piece Shipment API: POST /track/v1/associatedshipments.
 *
 * @see https://developer.fedex.com/api/en-us/catalog/track/v1/docs.html
 */
class FedExAssociatedShipmentsService
{
    ...
    /**
     * @param  array<string, mixed>  $payload  FedEx request body (associatedType, masterTrackingNumberInfo, etc.)
    ...
    public function track(array $payload, ?string $customerTransactionId = null, ?string $locale = null): array
    {
        ...
        try {
            ...
            Log::error('FedEx OAuth failed before associated shipments track.', [
        ...
        $response = FedExHttp::pending($timeout)
            ...
            ->withToken($token)
            ->post($base.'/track/v1/associatedshipments', $payload);
        ...
        if (! $response->successful()) {
            ...
            Log::error('FedEx associated shipments track HTTP error.', [
                'x-customer-transaction-id' => $txnHeader,
    ...
    private function firstFedExErrorMessage(array $normalizedErrors, array $body, int $httpStatus): string
    {
        ...
        return 'FedEx associated shipments track failed (HTTP '.$httpStatus.').';
    }
}
