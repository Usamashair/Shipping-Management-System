...
/**
 * Proxies FedEx Track by Tracking Number API: POST /track/v1/trackingnumbers.
 *
 * @see https://developer.fedex.com/api/en-us/catalog/track/v1/docs.html
 */
class FedExTrackByTrackingNumberService
{
    ...
    /**
     * @param  array<string, mixed>  $payload  FedEx request body (includeDetailedScans, trackingInfo, etc.)
    ...
    public function track(array $payload, ?string $customerTransactionId = null, ?string $locale = null): array
    {
        ...
        try {
            ...
            Log::error('FedEx OAuth failed before track by tracking number.', [
        ...
        $response = FedExHttp::pending($timeout)
            ...
            ->withToken($token)
            ->post($base.'/track/v1/trackingnumbers', $payload);
        ...
        if (! $response->successful()) {
            ...
            Log::error('FedEx track by tracking number HTTP error.', [
                'x-customer-transaction-id' => $txnHeader,
    ...
    private function firstFedExErrorMessage(array $normalizedErrors, array $body, int $httpStatus): string
    {
        ...
        return 'FedEx track by tracking number failed (HTTP '.$httpStatus.').';
    }
}
