...
/**
 * Proxies FedEx Send Notification API: POST /track/v1/notifications.
 *
 * @see https://developer.fedex.com/api/en-us/catalog/track/v1/docs.html
 */
class FedExTrackingNotificationService
{
    ...
    /**
     * @param  array<string, mixed>  $payload  FedEx request body (senderContactName, trackingEventNotificationDetail, etc.)
     * @return array<string, mixed>
     */
    public function send(array $payload, ?string $customerTransactionId = null, ?string $locale = null): array
    {
        ...
        try {
            ...
            Log::error('FedEx OAuth failed before tracking notification send.', [
        ...
        $response = FedExHttp::pending($timeout)
            ...
            ->withToken($token)
            ->post($base.'/track/v1/notifications', $payload);
        ...
        if (! $response->successful()) {
            ...
            Log::error('FedEx tracking notification HTTP error.', [
                'x-customer-transaction-id' => $txnHeader,
    ...
    private function firstFedExErrorMessage(array $normalizedErrors, array $body, int $httpStatus): string
    {
        ...
        return 'FedEx tracking notification request failed (HTTP '.$httpStatus.').';
    }
}
